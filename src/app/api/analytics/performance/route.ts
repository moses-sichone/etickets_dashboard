...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${process.env.BACKEND_API_URL}/api/analytics/performance/${queryString ? `?${queryString}` : ''}`
    ...
    if (!backendResponse.ok) {
      return NextResponse.json(
        { success: false, message: data.message || 'Failed to fetch analytics performance' },
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to fetch analytics performance' },
      { status: 500 }
    )
  }
}